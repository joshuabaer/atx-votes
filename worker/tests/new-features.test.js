...
describe("Novelty tone warning banner", () => {
  it("renders amber warning banner for Swedish Chef (reading level 6)", () => {
    expect(APP_JS).toContain("S.readingLevel===6||S.readingLevel===7||S.readingLevel===8");
  ...
  it("shows chef emoji for Swedish Chef mode", () => {
    // The chef emoji is encoded as unicode escape
  ...
  it("shows US flag emoji for Trump mode", () => {
    expect(APP_JS).toContain("\\uD83C\\uDDFA\\uD83C\\uDDF8");
  });

  it("shows compact one-line warning on race detail view", () => {
    expect(APP_JS).toContain("Viewing in Swedish Chef mode. Switch to Standard for neutral presentation.");
    expect(APP_JS).toContain("Viewing in Cowboy mode. Switch to Standard for neutral presentation.");
    expect(APP_JS).toContain("Viewing in Trump mode. Switch to Standard for neutral presentation.");
  });
...
// ===========================================================================
// 7. max_tokens reduction (2048 English, 4096 Spanish cached, 8192 Spanish fresh)
// ===========================================================================
describe("max_tokens reduction by language mode", () => {
  it("uses 2048 max_tokens for English", () => {
    const guideSrc = readFileSync(join(__dirname, "../src/pwa-guide.js"), "utf-8");
    // callClaude uses: lang === "es" ? 8192 : (lang === "es_cached" ? 4096 : 2048)
