...
describe("Reading level selection", () => {
  ...
  it("easter egg tones 6/7 are hidden by default", () => {
    const el6 = document.querySelector('[data-action="select-tone"][data-value="6"]');
    const el7 = document.querySelector('[data-action="select-tone"][data-value="7"]');
    expect(el6).toBeNull();
    expect(el7).toBeNull();
  });

  it("easter egg tones appear after 5 chef taps", () => {
    // The header "Talk to me like..." has data-action="chef-tap"
    for (let i = 0; i < 5; i++) {
      const tap = document.querySelector('[data-action="chef-tap"]');
      if (tap) tap.click();
    }
    const el6 = document.querySelector('[data-action="select-tone"][data-value="6"]');
    const el7 = document.querySelector('[data-action="select-tone"][data-value="7"]');
    expect(el6).not.toBeNull();
    expect(el7).not.toBeNull();

    el6.click();
    expect(S().readingLevel).toBe(6);
  });

  it("sets reading level 7 (Texas Cowboy) via easter egg", () => {
    for (let i = 0; i < 5; i++) {
      const tap = document.querySelector('[data-action="chef-tap"]');
      if (tap) tap.click();
    }
